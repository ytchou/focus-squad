...
- [ ] Set up Supabase project and run initial migration
- [ ] Configure environment variables (.env from .env.example)
- [ ] Implement Supabase Auth integration in FastAPI
- [ ] Create JWT validation middleware
- [ ] Set up Redis connection for session state

### User System (Backend)
- [ ] Implement `UserService` with CRUD operations
- [ ] Complete `GET /api/v1/users/me` endpoint
- [ ] Complete `PATCH /api/v1/users/me` endpoint
- [ ] Complete `GET /api/v1/users/{user_id}` public profile endpoint
- [ ] Add user creation on first OAuth login
