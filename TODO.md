...
- [ ] **Dashboard crash: `weekly_streaks` table not found**: `streak_service.get_weekly_streak()` crashes because migration `028_diary_integration.sql` hasn't been pushed to Supabase yet. This kills the entire `/dashboard/init` endpoint (all 4 parallel queries fail together). Fix: push pending migrations before alpha launch.
