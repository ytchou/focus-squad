...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def _credit_not_found(request: Request, exc: CreditNotFoundError):
        return error_response(404, str(exc) or "Credit record not found.", "CREDIT_NOT_FOUND")

    @app.exception_handler(InsufficientCreditsError)
    async def _insufficient_credits(request: Request, exc: InsufficientCreditsError):
    ...
    async def _gift_not_allowed(request: Request, exc: GiftNotAllowedError):
        return error_response(
            403,
    ...
    async def _gift_limit(request: Request, exc: GiftLimitExceededError):
        return error_response(
            429,
    ...
    async def _reflection_session_not_found(request: Request, exc: ReflectionSessionNotFoundError):
        return error_response(404, "Session not found.", "SESSION_NOT_FOUND")

    @app.exception_handler(NotSessionParticipantError)
    async def _not_participant(request: Request, exc: NotSessionParticipantError):
    ...
    async def _red_reason_required(request: Request, exc: RedReasonRequiredError):
        return error_response(
            422, "Red ratings require at least one reason.", "RED_REASON_REQUIRED"
        )

    @app.exception_handler(SessionNotRatableError)
    async def _session_not_ratable(request: Request, exc: SessionNotRatableError):
    ...
    async def _invalid_rating_target(request: Request, exc: InvalidRatingTargetError):
        return error_response(400, str(exc), "INVALID_RATING_TARGET")

    @app.exception_handler(RatingAlreadyExistsError)
    async def _rating_exists(request: Request, exc: RatingAlreadyExistsError):
        return error_response(
            409, "You have already rated participants in this session.", "RATING_ALREADY_EXISTS"
        )

    @app.exception_handler(NoPendingRatingsError)
    async def _no_pending_ratings(request: Request, exc: NoPendingRatingsError):
        return error_response(
    ...
    async def _livekit_error(request: Request, exc: LiveKitServiceError):
