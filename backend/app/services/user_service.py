...
class UserService:
    ...
    def _create_credits_with_retry(self, user_id: str, max_attempts: int = 5) -> None:
        """
        ...
        Args:
            ...
            max_attempts: Max insert attempts before raising error
        ...
        for _attempt in range(max_attempts):
            try:
                self.supabase.table("credits").insert(credits_data).execute()
                return  # Success
        ...
        raise UserServiceError(
            f"Unable to create credits record after {max_attempts} attempts due to "
            f"referral code collision: {last_error}"
        )
    ...
    def _create_associated_records(self, user_id: str) -> None:
        ...
        """
        # Create credits record with retry for referral code collision
        # referral_code is auto-generated by DB trigger, but may collide
        self._create_credits_with_retry(user_id)
